...
import { SafetyUrl } from "../../../Constants";
import { FeatureCollection, ReportType } from "../../../FormTypes";

export const SubmitSafetyReport = async (report: SafetyFields) => {
    const data = {
        concern_type: report.safetyType,
        concern_with: report.safetySubtype,
    ...
    try {
        const response = await fetch(SafetyUrl, options);
...
export const GetSafetyFeatureCollection = async (): Promise<FeatureCollection> => {
    const response = await fetch(`${SafetyUrl}`);

    if (response.ok) {
        return await response.json() as FeatureCollection;
    } else {
        console.log(`An error occurred while fetching the Safety features: ${response.status} - ${response.statusText}`);
        return {
